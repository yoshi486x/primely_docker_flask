...
cat = CATEGORIES[1]
df = create_table(cat)
ddf = pd.pivot_table(df, index='type', columns='date', values='value', fill_value=0)
# ddf


# %%
# Rename index in english
d_ja_row = ddf.index.values.tolist()
d_ja_row
D_JA_ROW = ['その他控除(月例･一時)', '住民税', '健康保険料', '厚生年金保険料', '所得税', '社食利用料', '還付徴収額', '雇用保険料']
D_EN_ROW = ['other_ded', 'resid_tax', 'health_ins', 'pension_ins', 'income_tax', 'cafe_use', 'refund_bil', 'emp_ins' ]

dconvTable = dict(zip(D_JA_ROW, D_EN_ROW))
print(dconvTable)
sddf = ddf.rename(index=dconvTable)
sddf


# %%
# Tweek values
ddemoTable = sddf.copy()
en_row = ddemoTable.index.values.tolist() # EN_ROW
WEIGHT = [0.0389, 0.023, 0.0045, 0.0029, 0.0122, 0.0312, 0.012, 0.033]

weiTable = dict(zip(D_EN_ROW, WEIGHT))

for key, val in weiTable.items():
    print(key, val)
    ddemoTable.loc[key] = (ddemoTable.loc[key] * val).astype(int).round(0)
ddemoTable
...
# print('cat:', cat)
# print('ddf:', ddf)
response = updateRes(Response, cat, ddf)
response
